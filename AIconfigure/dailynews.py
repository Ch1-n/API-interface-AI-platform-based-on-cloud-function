...
def dailynews(key):
    ...
    url = f"http://api.tianapi.com/txapi/bulletin/index?key={key}"
    try:
        req = requests.get(url=url)
        req_json = req.json()
        content["mtime"] = req_json['newslist'][0]['mtime']
        content["title"] = req_json['newslist'][0]['title']
        content["digest"] = req_json['newslist'][0]['digest']
        return content
...
# 每日新闻
dailynews = do.dailynews('5ca24f4c5c87879182b6c48d8352e0c2')
news_msg = f"每日新闻简报：\n {dailynews['mtime']} \n {dayilnews['title']} \n --{dayilnews['digest']}"
do.send_msg(news_msg)
print("每日简报推送完成")
time.sleep(5)
"""
...
def morning(loca_time, key):
    ...
    try:
        if loca_time > 5 and loca_time < 12:  # 早上
